...
#### 77. INTRODUCTION TO BOOTSTRAP PART 1 VIDEO - hands-on bootstrap setup (Step 501)

This assignment involved a hands-on tutorial to introduce the Bootstrap framework. The student began by downloading a specific version of Bootstrap and setting up a project folder, then created an `index.html` file to link the framework's CSS. The student successfully followed the tutorial to create a basic responsive webpage, observing how Bootstrap's pre-written code automatically styled an `<h1>` element. A crucial part of this process was a real-world debugging session where the student identified and reconciled unexpected broken image links, correctly deducing they were a side effect of the specific `bootstrap.css` file rather than a mistake in their own code. This demonstrated strong problem-solving skills and a practical understanding of how a website's files interact. The student also learned how Bootstrap uses modern CSS units like `rem` and `vw` to achieve responsiveness and how the viewport meta tag is essential for enabling this behavior.
